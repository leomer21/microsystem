...
@if(isset($top_branches_revenue) && App\Settings::where('type', 'commercial_enable')->value('state') == 1)
    <?php
    $AllBranchesRevenue=App\History::where('operation','Charged card')->orWhere('operation','reseller_charge_package')->orWhere('operation','paypal_charge_card')->sum('package_price'); if(!isset($AllBranchesRevenue)){$AllBranchesRevenue=0;}
...
<!--   ----------------------------------------------------------------------------------------------------- -->
            ...
            marketing_campaigns_options = {
                ...
                legend: {
                    orient: 'vertical',
                    x: 'left',
                    data: ['Email Campaigns', 'SMS Campaigns', 'Local Messages', 'Push Notifications']
                },
                ...
                series: [{
                    ...
                    data: [
                        ...
                        {value: {{ App\History::where('operation','pushNotification_campaign')->whereBetween('add_date',[$statisticsStartDate, $statisticsEndDate])->count() }}, name: 'Local Messages'},
                        {value: {{ App\History::where('operation','localMessages_campaign')->whereBetween('add_date',[$statisticsStartDate, $statisticsEndDate])->count() }}, name: 'Push Notifications'}
                    ]
                }]
            };
