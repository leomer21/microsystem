<!-- Scrollable datatable -->
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Offer Codes</h5>
        </div>
...
<script>
    ...
    var dataSet = [
        @foreach($offers as $value)
         ...
         $adminName = App\Admins::where('id', $value->a_id)->value('name'); 
         ?>
        
        @if(isset($users))
            ['{{ $value->created_at }}','{{ $users->u_name }}' , '{{ $users->u_phone }}','{{ $value->offer_code }}','@if($value->state == 1) Delivered @else <button type="button" class="btn btn-danger btn-ladda btn-ladda-spinner" data-spinner-color="#333" data-style="radius" style="width: 91px;"><span class="ladda-label">Pending</span></button>@endif','@if(isset($value->a_id)) {{ $adminName }} @else None @endif', '{{ $value->updated_at }}'],
        @else
            ['','Deleted ( User ID: {{$value->u_id}})','','','','',''],
        @endif
        <?php unset($adminName); ?>
        @endforeach
    ];
