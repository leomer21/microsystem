...
Suprabuilder.prototype = {
    ...
    , _initTypography: function() {
        ...
        options.typographyGrid.forEach(function(typographyItem, indx) {
            ...
            var fontFamily = option.dataset.value;

            if (_this.arrayFontsOnProject.indexOf(fontFamily + ':' + option.dataset.weight) === -1) {
                _this.arrayFontsOnProject.push(fontFamily + ':' + option.dataset.weight);
            }
            button.children[0].innerHTML = option.innerHTML;
            button.style.fontFamily = fontFamily;
            button.dataset.value = fontFamily;
            button.dataset.weight = option.dataset.weight;
            button.style.fontWeight = option.dataset.weight;
    ...
    , _createControlElements: function(editElement, domIdentif, editType, classNameAdd, li, el) {
        ...
        this._setPropertyForWrapper(el, wrap);
    ...
    , _setPropertyForWrapper: function(el, wrap) {
        var _this = this;
        ...
        //if (el.nodeName === 'I') el.style.display = 'block';
    ...
    , _findElForOptions: function(DOM) {
        var _this = this;
        var options = DOM.querySelectorAll('.bg-video, .spr-gallery, form, .parallax, .single-iframe-popup, .smooth, .spr-magnific');
    ...
    , _reloadVideoBg: function (page, mode) {
        ...
        if (vides) {
            Array.prototype.forEach.call(vides, function(vide) {
                vide.innerHTML = '';
                if (mode === 'run') vide_run();
            });
        }
    }
    /**
    ...
    , _clearOptionClasses: function(page) {
        var options = page.querySelectorAll('[class*=spr-]');
        Array.prototype.forEach.call(options, function(el) {
            el.className = el.className.replace(/(\s?spr-[^\s]*)+/i, '').trim();
    ...
    , applyMagnificPopup: function(DOMEelement) {
        ...
        if (DOMEelement.tagName === 'A' && DOMEelement.href.search(/player\.vimeo\.com|embed/i) === -1) {
    ...
    , ajax: function(form, urlAjax, callback) {
        ...
        var method = 'POST';
        var url = '/builder/ajax.php?mode=' + urlAjax;
