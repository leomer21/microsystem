...
// step A ( Get commercial values from DB )
    ...
    // check if user login by mac ( if login by mac we should add speed and quota value with the mac address )
if( $mac == $user ){$db_user=$mac;}

$monthly_package_id=$row_checkUser['monthly_package_id'];
if($monthly_package_id){$packageMode="monthly";}

$validity_package_id=$row_checkUser['validity_package_id'];
if($validity_package_id){$packageMode="validity";}

$time_package_id=$row_checkUser['time_package_id'];
if($time_package_id){$packageMode="time";}

$bandwidth_package_id=$row_checkUser['bandwidth_package_id'];
$bandwidth_package_start=$row_checkUser['bandwidth_package_start'];
$bandwidth_package_expiry=$row_checkUser['bandwidth_package_expiry'];


	if($packageMode=="monthly") // Monthly package
	{
        //include 'monthly.php';
        $package_id=$row_checkUser['monthly_package_id'];
        $package_start=$row_checkUser['monthly_package_start'];
        $package_expiry=$row_checkUser['monthly_package_expiry'];
    }
	elseif($packageMode=="validity") // Validity package
	{
        //include 'validity.php';
        $package_id=$row_checkUser['validity_package_id'];
        $package_start=$row_checkUser['validity_package_start'];
        $package_expiry=$row_checkUser['validity_package_expiry'];
    }
	elseif($packageMode=="time") // Time package
	{
        //include 'period.php';
        $package_id=$row_checkUser['time_package_id'];
        $package_start=$row_checkUser['time_package_start'];
        $package_expiry=$row_checkUser['time_package_expiry'];
    }
    else{
        // user didn't charge any internet package before
        if($open_system==2){$goFreeInternet="yes";}
        else{echo "RejectA1";} // Not found user mode
...
if($packageMode) {
    ...
    // package table
    $getPackageData = "select * from `packages` where `id`='$package_id'";
    ...
    if ($final_validate_date_charging_x_x_x >= 0) { // user have remaining days
        ...
        if ($quota_limit_upload or $quota_limit_download or $quota_limit_total or $packageMode=="time") {
            ...
            //if ($userLoginByMacAuto == "yes") {$getAllUserRecords = "SELECT * FROM `radacct` WHERE `username` = '$db_user' AND `acctstarttime` >= '$package_start'";} // Normal Case
            //else {$getAllUserRecords = "SELECT * FROM `radacct` WHERE `username` = '$user' AND `acctstarttime` >= '$package_start'";}

            // after add user ID field in radacct table
            $getAllUserRecords="SELECT * FROM `radacct` WHERE `u_id` = '$db_Uid' AND `acctstarttime` >= '$package_start'";
            ...
            if($packageMode=="time"){
                if($session_time and $session_time!=0){
                    ...
                    if($final_session_time<=0){$rejected="yes";} // Reject Reason : session time ended
                }
            }
        ...
        if (!isset($rejected) or $rejected != "yes") {
            ...
            if ($endLimitOverwrited != "yes") {
                ...
                if($packageMode=="time") {$session_time=$final_session_time;}
                else{$session_time = 0;}
...
// check if user have credit for auto charging
if($checkOrderChargePackage=="yes"){
    // check if user have credit equal package credit ???
    // if have exec on api charge ???
}


    //if ($db_uState=="0" or $checkOrderChargePackage=="yes"){
    //
    //	// step 1 : check if user have order charge package
    //	$getOrderChargePackage="select `packageID`,`packageType`,`id` from `order_charge_package` where `userID`='$db_Uid' and `state`='active' order by `id` ASC ";
    //	$r_getOrderChargePackage=@mysqli_query($conn,$getOrderChargePackage);
    //	if(@mysqli_num_rows($r_getOrderChargePackage)>0){
    //
    //		$row_getOrderChargePackage=@mysqli_fetch_array($r_getOrderChargePackage);
    //		$packageID=$row_getOrderChargePackage['packageID'];
    //		$packageType=$row_getOrderChargePackage['packageType'];
    //		$packageIdInOrderCharge=$row_getOrderChargePackage['id'];
    //
    //		if($packageType=="monthly"){include 'orderChargePackageValidity.php';}// validity package
    //		if($packageType=="monthly2"){include 'orderChargePackageMonthly.php';}// Monthly package
    //
    //	}else{// user inactive and not have any order_charge_package
    //		if($open_system=="yes" and $userCanBackToTrialMode=="yes"){$goFreeInternet="yes";}// last chance if user can back to free internet
    //		else{echo "Reject";}// user inactive and not have any order_charge_package
    //	}
    //
    //	//include 'orderChargePackageValidity.php';
    //}
