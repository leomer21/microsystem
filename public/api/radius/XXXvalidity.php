...
// user table
$validity_package_start=$row_checkUser['validity_package_start'];
$validity_package_expiry=$row_checkUser['validity_package_expiry'];

// package table
$getPackageData="select * from `packages` where `id`='$validity_package_id'";
...
if($quota_limit_upload or $quota_limit_download or $quota_limit_total){// if user have quota limit
    if($bandwidth_package_id){// if user have bandwidth package
        $bandwidth_package_before_convert= strtotime("$validity_package_expiry");
        ...
        if($final_sub_bandwidth_date>=0){// user have remaining days in bandwidth package
            ...
            $validity_package_start=$bandwidth_package_start;
...
$nowdate_charging_x_x_x = strtotime("$validity_package_expiry");
...
// step 3 : check if user have limit
    if($quota_limit_upload or $quota_limit_download or $quota_limit_total)
    {
        ...
        if($userLoginByMacAuto=="yes"){$getAllUserRecords="SELECT * FROM `radacct` WHERE `username` = '$db_user' AND `acctstarttime` >= '$validity_package_start'";}
        // Normal Case
        else{$getAllUserRecords="SELECT * FROM `radacct` WHERE `username` = '$user' AND `acctstarttime` >= '$validity_package_start'";}
