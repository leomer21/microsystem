	 ...
	 { 
		...
		if($sys_local_or_web=="local")
		{
			if($me_local_or_web=="local"){$can_go="yes";}
				...
				$url = $local_url."?op=mobileCards2";
				//$data='{"username":"test","password":"test","web":"http://m","lang":"ar","CardType":"Vodafone","CardPrice":"0.50"}';
				...
				, "CardType" => "$cardType", "CardPrice" => "$CardPrice", "ChargeType" => "$chargeType", "ChargeFor" => "$chargeFor");                                                                    
		...
		if(@mysql_num_rows($r_get_user_data)>0)
		{
			...
			$network_name=$row_get_user_data['network_name'];
...
if($cardType=="Vodafone" and $CardPrice=="0.50"){$action_type="vod1";}
if($cardType=="Vodafone" and $CardPrice=="1.25"){$action_type="vod2";}
if($cardType=="Vodafone" and $CardPrice=="2.50"){$action_type="vod3";}
...
if($payment_type=="special_cards")// Ì⁄‰Ï ‘«—Ì ﬂ—Ê  ›ﬁÿ „‘ —’Ìœ ‘«„·
	{
		...
		if($card__type__=="ma"){$final_card_type="Marhaba";}
			...
			{ 
				...
				$for_show__card_no=$row_get_selected_card['card_no'];
				$for_show__card_serial=$row_get_selected_card['card_serial'];
				$for_show__expire_date=$row_get_selected_card['expire_date'];
				$for_show__card_type=$row_get_selected_card['card_type'];
				...
				if ($total_final_salary<=$current_user_creadit){
			...
			$disable_card="update mob_cards set state='inactive',user_charge_id='$db_id',user_charge_uname='$db_u_uname',user_charge_name='$db_u_name'
			,network_admin_id='$current_get_customer_id',network_admin_row_id='$c_row_id',network_admin_name='$customer_name'
			,network_admin_uname='$customer_user',network_admin_url='$customer_url',network_code='$network_code',network_name='$network_name'
			,charge_date='$today',charge_time='$today_time',charge_salary='$total_final_salary',charge_cost='$total_cost',user_ip='$user_ip'
			where id='$selected_card_id'";
			@mysql_query($disable_card,$conn_sys2)or die(mysql_error());
			
			/////////////////////////////
			// first step 2 Œ’„ ”⁄— «·ﬂ«—  ÊﬁÌ„ … „‰ »Ì«‰«  «·⁄„Ì· ›Ï «·‘—ﬂ…
			/////////////////////////////
			
			$get_a_moment_customer_data="select * from customers_mob_data where customer_id='$current_get_customer_id'";
			$r_get_a_moment_customer_data=@mysql_query($get_a_moment_customer_data,$conn_sys2);
			$row_get_a_moment_customer_data=@mysql_fetch_array($r_get_a_moment_customer_data);
			$a_moment_cards_creadit=$row_get_a_moment_customer_data["$action_type"];
			$a_moment_creadit=$row_get_a_moment_customer_data['creadit'];
			$variable_="fee_"."$action_type";
			$a_moment_cards_fee=$row_get_a_moment_customer_data["$variable_"];
			$a_moment_cards_creadit_after_discount=$a_moment_cards_creadit-1;
			$a_moment_creadit_after_discount=$a_moment_creadit-($a_moment_cards_fee+$selected_card_amount);
			
			$update_customers_mob_data_after_discount="update customers_mob_data set creadit='$a_moment_creadit_after_discount', 
			$action_type='$a_moment_cards_creadit_after_discount' where customer_id='$current_get_customer_id'";
			@mysql_query($update_customers_mob_data_after_discount,$conn_sys2)or die(mysql_error());
			
			/////////////////////////////
			// first step 3 Œ’„ «·„»·€ „‰ «·⁄„Ì·
			/////////////////////////////
			
			$user_creadit_after_discount=$current_user_creadit-$total_final_salary;
			$discount_user_creadit="update users set creadit='$user_creadit_after_discount' where u_id='$db_id'";
			@mysql_query($discount_user_creadit,$conn_user)or die(mysql_error());
			
			/////////////////////////////
			// first step 4 ≈÷«›… «·«—»«Õ ›Ï Õ”«» ’«Õ» «·‘»ﬂ…
			/////////////////////////////
			
			$current_income_id=rand(11165,99999);				
			$today_all=$today." ".$today_time;
			$add_income_name="ﬁ«„ «·⁄„Ì· $db_u_uname »‘—«¡ ﬂ«—  $final_card_type »ﬁÌ„… $selected_card_amount  ﬂ·›… «·ﬂ«—  $total_cost «Ã„«·Ï ”⁄— «·ﬂ«—  $total_final_salary";
			$insert_mobth_income_name="insert into $month_table_name (month_type,month_income_name,month_income_salary,month_income_date,month_income_id) values ('income','$add_income_name',$total_final_salary,'$today_all',$current_income_id)";
			@mysql_query($insert_mobth_income_name,$conn_user);
			
			/////////////////////////////
			// first step 5 ≈œŒ«· «·ÂÌ” Ê—Ï ›Ï ”Ì” „ «·⁄„Ì· Ê”Ì” „ «·‘—ﬂ…
			/////////////////////////////
			
			$insert_history_in_company="insert into history (type,value1,value2,value3,value4,value5,value6,value7,value8) values
			('new_mobile_card_pay','$final_card_type','$total_final_salary','$current_get_customer_id','$db_u_name','$today','$today_time','$user_ip','$customer_url') ";
			@mysql_query($insert_history_in_company,$conn_sys2)or die(mysql_error());
			
			$insert_history_in_customer_sys="insert into history (add_date,add_month,add_time,type1,type2,operation,details,notes,u_id,u_name,u_uname) values
	        ('$today','$month_table_name','$today_time','microcharge','user','new_mobile_card_pay','$final_card_type','$total_final_salary','$db_id','$db_u_name','$db_u_uname')";
			@mysql_query($insert_history_in_customer_sys,$conn_user)or die(mysql_error());
			
			/////////////////////////////
			// first step 6 ⁄—÷ »Ì«‰«  «·ﬂ«— 
			/////////////////////////////
			$get_selected_card2="select * from mob_cards where card_type='$final_card_type' and amount='$card_cost' and id='$selected_card_id' order by id ASC";
			$r_get_selected_card2=@mysql_query($get_selected_card2,$conn_sys2);
			$for_show_row_get_selected_card=@mysql_fetch_array($r_get_selected_card2);
				
			$for_show_card_no_splited = str_split($for_show__card_no,4);
			if($for_show__card_type=="Vodafone"){$for_show__mobile_network_name_ar="‘»ﬂ… ›Êœ«›Ê‰"; $final_card_type="Vodafone"; $show_charge_way="* 858 * $for_show_card_no_splited[0] $for_show_card_no_splited[1] $for_show_card_no_splited[2] $for_show_card_no_splited[3] #";  $show_charge_way_for_dial="*858*$for_show__card_no#";}
			if($for_show__card_type=="Mobinil"){$for_show__mobile_network_name_ar="‘»ﬂ… „Ê»Ì‰Ì·"; $final_card_type="Mobinil"; $show_charge_way="# 120 * $for_show_card_no_splited[0] $for_show_card_no_splited[1] $for_show_card_no_splited[2] $for_show_card_no_splited[3] #";    $show_charge_way_for_dial="#120*$for_show__card_no#";}
			if($for_show__card_type=="Etisalat"){$for_show__mobile_network_name_ar="‘»ﬂ… « ’«·« "; $final_card_type="Etisalat"; $show_charge_way="* 556 * $for_show_card_no_splited[0] $for_show_card_no_splited[1] $for_show_card_no_splited[2] $for_show_card_no_splited[3] #";   $show_charge_way_for_dial="*556*$for_show__card_no#";}
			if($for_show__card_type=="Marhaba"){$for_show__mobile_network_name_ar="ﬂ«—  „—Õ»«"; $final_card_type="Marhaba"; $show_charge_way="$for_show_card_no_splited[0] $for_show_card_no_splited[1] $for_show_card_no_splited[2] $for_show_card_no_splited[3]";                $show_charge_way_for_dial="$for_show__card_no";}
			
			if($login_lang=="ar"){$for_show__mobile_network_name=$for_show__mobile_network_name_ar;}
			else{$for_show__mobile_network_name=$final_card_type;}
			...
			, "TransNo" => "$selected_card_id", "cardType" => "$final_card_type", "cardTypeForView" => "$for_show__mobile_network_name"
			,"cardNoForDial" => "$show_charge_way_for_dial", "cardNoForView" => "$show_charge_way", "cardNoOnly" => "$for_show__card_no"
			, "cardSerial" => "$for_show__card_serial", "cardAmount" => "$selected_card_amount", "totalFees" => "$total_cost"
			, "totalCost" => "$total_final_salary", "userCreaditAfterCharge" => "$user_creadit_after_discount");                                                                    
			...
			//////// -- == -- /////////  -- == --  /////////  -- == --  /////////  -- == --  /////////  -- == --  /////////    
				...
				$status=0;if($login_lang=="ar"){$status_message=$errorMobileDontHaveCreadit_ar;}else{ $status_message=$errorMobileDontHaveCreadit_en;}
				$data_before = array("status" => "$status", "statusMessage" => "$status_message");                                                                    
				echo $data = json_encode($data_before);	
				}
			...
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				$insert_history_in_company="insert into history (type,value1,value2,value3,value4,value5,value6) values('not_found_mobile_card','$final_card_type','$CardPrice','$current_get_customer_id','$uname','$today','$today_time') ";
				@mysql_query($insert_history_in_company,$conn_sys2)or die(mysql_error());}
