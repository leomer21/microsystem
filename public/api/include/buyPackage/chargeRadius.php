...
if(@mysqli_num_rows($r_getCustomerDB)>0) {
    ...
    if (@mysqli_num_rows($result_select_current_user_data) > 0) {
        ...
        if ($reseller) {
            ...
            if($package_price == 0) {
                $haveCredit = 1;
            }else if ($package_price > $current_user_credit) {
                $haveCredit = 0;
            } else {
                $haveCredit = 1;
            }
        ...
        // check if PMS integration is on (in this user is post paid)
        $getPmsState = "select * from $customerDatabase.settings where `type`='pms_integration'";
        $r_getPmsState = @mysqli_query($conn_microsystem,$getPmsState);
        $row_ResellerDate = @mysqli_fetch_array($r_getPmsState);
        if(isset($row_ResellerDate)){
            $pmsState = $row_ResellerDate['state'];
            if($pmsState == "1"){
                // PMS integration is on, so will check if user registerd by PMS or from complemntry signup
                if( $row_result_select_current_user_data['pms_id'] != "0" ){
                    // check if user didnt checked out (not suspended) to be able to send the invoice to the PMS
                    if($row_result_select_current_user_data['suspend'] == "0"){
                        // so this user is registerd from any on the PMS integrations, and still checked in, so we will apply postpaid invoiceing and send this pill to the PMS
                        $haveCredit = 1;
                        $userPmsId = $row_result_select_current_user_data['pms_id'];
                        $room_no = $row_result_select_current_user_data['pms_room_no'];
                        $canChargePMS=1;
                        // ** post pound to the last step to avoid invoice dublicate. ** //
                        // // insert new invoice into our database table to prepare it for sending to the PMS
                        // @mysqli_query($conn_microsystem,"insert into $customerDatabase.pms_invoices (`user_id`,`pms_id`,`package_id`,`package_name`,`price`,`room_no`,`created_at`) values ('$u_id_charging','$userPmsId','$package_id','$package_name','$package_price','$room_no','$created_at')");
        ...
        if ($haveCredit != 1) {
            ...
            { 
                if(isset($canChargePMS) and $canChargePMS==1){
                    // insert new invoice into our database table to prepare it for sending to the PMS
                    @mysqli_query($conn_microsystem,"insert into $customerDatabase.pms_invoices (`user_id`,`pms_id`,`package_id`,`package_name`,`price`,`room_no`,`created_at`) values ('$u_id_charging','$userPmsId','$package_id','$package_name','$package_price','$room_no','$created_at')");
                }
