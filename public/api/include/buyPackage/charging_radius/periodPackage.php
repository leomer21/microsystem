...
if ($datediff_charging_x_x_x >= 0 and isset($time_package_expiry)) {
...
if ($stop_working!="yes") {
    ...
    // Get expiration date
    if($packageTime_package_expiry == "1"){
        $card_validate_date_charging = date('Y-m-d');
    }else{
        $card_validate_date_charging_before_convert = strtotime(date("$today_full24", strtotime($date)) . " +$packageTime_package_expiry day");
    ...
    $updateUserDetails = "update $customerDatabase.users set
        ...
        `time_package_id`='$package_id',
        `time_package_start`='$created_at',
        `time_package_expiry`='$card_validate_date_charging',
		`group_id`='$group_id',
