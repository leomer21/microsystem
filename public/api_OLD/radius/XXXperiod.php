...
// user table
$time_package_start=$row_checkUser['time_package_start'];
$time_package_expiry=$row_checkUser['time_package_expiry'];

// package table
$getPackageData="select * from `packages` where `id`='$time_package_id'";
...
if($quota_limit_upload or $quota_limit_download or $quota_limit_total){// if user have quota limit
    if($bandwidth_package_id){// if user have bandwidth package
        $bandwidth_package_before_convert= strtotime("$time_package_expiry");
        ...
        if($final_sub_bandwidth_date>=0){// user have remaining days in bandwidth package
            ...
            $time_package_start=$bandwidth_package_start;
...
$nowdate_charging_x_x_x = strtotime("$time_package_expiry");
...
// step 3 : check if user have limit
    if($quota_limit_upload or $quota_limit_download or $quota_limit_total or $session_time)
    {

        // step 3.A : get login history

        // user logged in by new feature ( auto insert mac ) and logged in automatically
        if($userLoginByMacAuto=="yes"){$getAllUserRecords="SELECT * FROM `radacct` WHERE `username` = '$db_user' AND `acctstarttime` >= '$time_package_start'";}
        // Normal Case
        else{$getAllUserRecords="SELECT * FROM `radacct` WHERE `username` = '$user' AND `acctstarttime` >= '$time_package_start'";}
...
// step 4 : complete login process if user have limit or not
    ...
    if(!isset($rejected) or $rejected!="yes"){
        ...
        if($endLimitOverwrited!="yes"){
            ...
            $session_time=$final_session_time;
...
// step 4.A get Radius type Attributes
        if(!$radius_type){$radius_type="mikrotik";}
