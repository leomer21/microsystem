	...
	require_once 'include/config.php';
		...
		if($sys_local_or_web=="local")
		{
			if($me_local_or_web=="local"){$can_go="yes";}
				...
				$url = $local_url."?op=getPackagesSMS";
				//$data='{"username":"гЌгѕ","password":"гЌгѕ","web":"http://m","lang":"ar"}';
				// send json
...
//////////////////////////////////////////////////////////////// Charge Card ////////////////////////////////////////////////////////////////////
	        		...
	        		,`limit_profile_id`,`offer`,`offer_start_date`,`offer_start_time`,`offer_end_date`,`offer_end_time` from packages where type='sms' and state='active' and network_code='$network_code' and hotspot_or_ppp='$hotspot_or_ppp' and limit_profile_id is null  ORDER BY id ASC";}
	        		else{$get_packages="select `id`,`network_code`,`name`,`type`,`price`,`period`,`speed_profile_id`
	        		,`limit_profile_id`,`offer`,`offer_start_date`,`offer_start_time`,`offer_end_date`,`offer_end_time` from `packages` where  type='sms' and `state`='active' and `network_code`='$network_code' and hotspot_or_ppp='$hotspot_or_ppp'  ORDER BY id ASC";}
...
//			        		$speed_profile_id=$row_get_all_network_active['speed_profile_id'];
			        		...
			        		///// speed_profile_id //  г «бџ«∆е« б«д е–… «б»«ё… ем »«ё… SMS 
...
//                            }//if($limit_profile_id)            
                            ...
                            if($currentOffer_status=="yes"){
								...
								$datediff_charging_for_reset_counter_QAQ = ($nowdate_charging_for_reset_counter_QAQ - $thendate_charging_for_reset_counter_QAQ);                 // бЎ—Ќ  «—нќ ёѕнг гд  «—нќ ћѕнѕ
								$final__for_reset_counter_QAQ=round($datediff_charging_for_reset_counter_QAQ/86400);
								//////////////////////////////////////////////////////////////////////////////////////
								if($final__for_reset_counter_QAQ>=0)// нЏдм б”е «бЏ—÷ гд е«‘
								{
									...
									if($timeDiff){$canDoIt="yes";}else{}// н»ём «бжё  ќб’
