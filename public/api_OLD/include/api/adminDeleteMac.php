...
//////////////////////////////////////////////////////////////// Charge Card ////////////////////////////////////////////////////////////////////
				...
				if($mac or $macID)
				{
						$checkIfMacExist="select `id` from `mac` where `mac`='$mac' or `id`='$macID'";
						$r_checkIfMacExist=@mysql_query($checkIfMacExist);
						if(@mysql_num_rows($r_checkIfMacExist)>0)
							{
								$row_select_Mac=@mysql_fetch_array($r_checkIfMacExist);
								$gettedid=$row_select_Mac['id'];
								
								$deleteMac="delete from `mac` where `id`='$gettedid'";
								@mysql_query($deleteMac);
								
								$status=1;if($login_lang=="ar"){$status_message=$macDeletedSuccessfully_ar;}else{ $status_message=$macDeletedSuccessfully_en;}
