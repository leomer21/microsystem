...
if ($datediff_charging_x_x_x >= 0 and isset($validity_package_expiry)) {
...
if ($stop_working!="yes") {
    ...
    // Get expiration date
    if($package_period == "1"){
        $card_validate_date_charging = date('Y-m-d');
    }else{
        $card_validate_date_charging_before_convert = strtotime(date("$today_charging", strtotime($date)) . " +$package_period day");
        $card_validate_date_charging = date('Y-m-d', $card_validate_date_charging_before_convert);
    }
    
    $card_validate_date_charging=$card_validate_date_charging.$today_time24;
    if($reseller)
    {
    ...
    $updateUserDetails = "update $customerDatabase.users set
        ...
        `validity_package_id`='$package_id',
        `validity_package_start`='$today_full24',
        `validity_package_expiry`='$card_validate_date_charging',
		`group_id`='$group_id',
        `monthly_package_id`=NULL,
        `monthly_package_start`=NULL,
        `monthly_package_expiry`=NULL,
