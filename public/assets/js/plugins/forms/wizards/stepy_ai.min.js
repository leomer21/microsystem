  ...
  var methods = {
    init: function(settings) {
      return this.each(function() {
        ...
        this.opt = $.extend({}, $.fn.stepy_ai.defaults, settings);
      ...
      self.steps.delegate('input[type="text"], input[type="password"]', 'keypress', function(evt) {
        ...
        if (key == 13) {
          ...
          var buttons = $(this).closest('fieldset').find('.stepy-navigator-ai');

          if (buttons.length) {
            var next = buttons.children('.button-next-ai');
      ...
      self.finish = (finish.length === 1) ? finish : that.children('.stepy-finish-ai');
      ...
      if (self.finish.length) {
        ...
        self.steps.last().children('.stepy-navigator-ai').append(self.finish);
      } else {
        $.error('Submit button or element with class "stepy-finish-ai" missing!');
      }
    }, _createBackButton: function(nav, index) {
      var self       = this,
          that       = $(this),
          attributes = { href: '#', 'class': 'button-back-ai', html: self.opt.backLabel };
      ...
      var step = $(step).attr('id', $(this).attr('id') + '-step-' + index).addClass('stepy-step-ai'),
      ...
      var self       = this,
          that       = $(this),
          attributes = { href: '#', 'class': 'button-next-ai', html: self.opt.nextLabel };
      ...
      return $('<div class="stepy-navigator-ai" />');
      ...
      return $(this).each(function() {
        ...
        if (that.data('stepy_ai')) {
          var steps = that.data('stepy_ai', false).children('fieldset').css('display', '');

          that.children('.stepy-errors').remove();
          this.finish.appendTo(steps.last());
          steps.find('p.stepy-navigator-ai').remove();
        }
      });
    }, step: function(index) {
      var self = this
