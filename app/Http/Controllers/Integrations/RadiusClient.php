...
class RadiusClient
{
                                ...
                                $accountingPort     = 1813)
    {
        ...
        $this->attributesInfo[44] = array('Acct-Session-Id', 'I');
        ...
        $this->attributesInfo[48] = array('Acct-Output-Octets', 'I');
        $this->attributesInfo[49] = array('Acct-Terminate-Cause', 'I');
        $this->attributesInfo[50] = array('Acct-Multi-Session-Id', 'I');
    ...
    public function getReadableReceivedAttributes()
    {
        ...
        if (isset($this->attributesReceived)) {
            ...
            foreach($this->attributesReceived as $receivedAttr) {
                ...
                $attributes[$counter]= array(sprintf('%s: ', $info[0]));

                if (26 == $receivedAttr[0]) {
                    $vendorArr = $this->decodeVendorSpecificContent($receivedAttr[1]);
                    foreach($vendorArr as $vendor) {
                        $arrayOfData = array('Vendor-Id'=> $vendor[0], 'Vendor-type'=> $vendor[1], 'value'=> $vendor[2]);
                        // $attributes[]= array_push($attributes[$counter],sprintf('Vendor-Id: %s, Vendor-type: %s, Attribute-specific: %s',$vendor[0], $vendor[1], $vendor[2]));
                        $attributes[]= array_push($attributes[$counter],$arrayOfData);
                    }
                } else {
                    $attributes[]= array_push($attributes[$counter],$receivedAttr[1]);
                }
    ...
    public function accountingRequest($value1 = '', $value2 = '', $value3 = '', $value4 = '')
    {
        $this->clearDataReceived()
             ->clearError()
        ...
        // if (0 < strlen($value1)) {
            $this->setAttribute(42, $value1); // Acct-Input-Octets
            $this->setAttribute(43, $value2); // Acct-Output-Octets
            $this->setAttribute(46, $value3); // Acct-Session-Time
            $this->setAttribute(47, $value4); // Acct-Input-Packets
        // }
