            ...
            if (isset($emailVerificationSwitchRoomType) && $emailVerificationSwitchRoomType == '1') {
                ...
                if($emailVerificationSwitchToGroupId != $userdata->group_id){
                    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                        $ChatGPT->sendEmailVerifyUsingChatGptWithoutWaiting($userdata->u_id,'login',$email,$userdata->u_name,$u_country);
                    }else{
                        $email = explode(",", $userdata->u_email);
                        $ChatGPT->sendEmailVerifyUsingChatGptWithoutWaiting($userdata->u_id,'login',$email[0],$userdata->u_name,$u_country);
            ...
            if ($canLogin == 1 and !isset($errorMessage)) // successfully login
            {
                ...
                if(App\Settings::where('type', 'pms_integration')->value('state')==1 && App\Settings::where('type', 'pms_save_email_from_login_page')->value('state')==1){
                    if( $request->input('emailForPms')!=""){
                        App\Users::where('u_id', $user_id)->update(['u_email' => $request->input('emailForPms')]);  
                    }
                }
        ...
        if (isset($checkIfUserExist) and isset($signup->network_id) and App\Settings::where('type', 'mergeAccounts')->value('state')!=1) {
            ...
            if( (App\Settings::where('type', 'mergeAccounts')->value('state')==1 and App\Users::where('u_phone', $request['countrycode'].$request['phone'])->count()>0 ) or ( App\Settings::where('type', 'mergeAccounts')->value('state')==1 and App\Users::where('u_email', $request['email'])->count() > 0) )
            {
                ...
                if ( $networkType == "0" or ( App\Settings::where('type', 'Accountkitappid')->value('state') == 1 and $networkType == "2" ) ) {
                    ...
                    $ChatGPT->sendEmailVerifyUsingChatGptWithoutWaiting($user_id,'signup',$userdata->u_email,$userdata->u_name,$u_country);
