...
{ 
                ...
                if($request->action == "welcome"){
                    ...
                    // if($session->is_admin == "1"){
                        if( isset($request->system) ){
                            ...
                            if($session->is_admin == "1"){ $data = DB::table("$request->system.radacct_active_users")->orderBy('radacctid','desc')->limit(10)->get(); }
                            else{ 
                            ...
                            // get unregisterd devices
                                $unregisterdDevices = DB::table($request->system.'.hosts')->where(['u_id'=>'0', 'internet_access'=>'0'])->orderby('id', 'desc')->limit(0)->get();
                    ...
                    if($session->is_admin == "1"){    
                        if( isset($request->system) ){
                            ...
                            $data = DB::table("$request->system.users")->where('u_name','like', '%'.$request->target.'%')->orWhere('pms_room_no', $request->target)->orderBy('u_id','desc')->limit(5)->get();
                            if(isset($data)){
                    ...
                    if($session->is_admin == "1"){    
                        if( isset($request->system) ){
                            ...
                            $data = DB::table("$request->system.users")->where('u_name','like', '%'.$request->name.'%')->orWhere('pms_room_no', $request->name)->orderBy('u_id','desc')->limit(5)->get();
                            if(isset($data)){
                        ...
                        if( isset($request->system) ){
                            ...
                            foreach ($data as $key => $value) {
                                ...
                                if(isset($downloadSpeed) and $downloadSpeed!=0){
                                    array_push($response, ['downloadSpeedBytes' => $downloadSpeed, 'uploadSpeedBytes' => $uploadSpeed, 'name'=> $value->u_name, 'mobile' => $value->pms_room_no, 'speed'=>$value->speed_rate, 'uptime'=>$value->uptime ]);
                                }
                            }
                            
                            // when concurrent more than 1000 we shoud disable the following function "responseTodayQuotaBefore"
                            ...
                            if(isset($TodayOnlineData) and count($TodayOnlineData) > 1){
                                ...
                                foreach ($usageTodaySorted as $key => $todayUser) {
                                    ...
                                    if($currUsageToday != 0) {
                                        array_push($responseTodayQuotaAfter, ['name' => $currUserData->u_name, 'mobile' => $currUserData->u_uname, 'dayQuotaUsage' => $currUsageToday.'GB' ]); 
                                    }
                                }
                            ...
                            if(isset($monthOnlineData) and count($monthOnlineData) > 1){
                                ...
                                foreach ($usageMonthSorted as $key => $MonthUser) {
                                    ...
                                    if($currUsageMonth != 0) { 
                                        if(isset($currUserData->u_name)) {$name = $currUserData->u_name; }else{$name = '';}
                                        if(isset($currUserData->u_uname)) {$mobile = $currUserData->u_uname; }else{$mobile = '';}
                                        array_push($responseMonthQuotaAfter, ['name' => $name, 'mobile' => $mobile, 'monthQuotaUsage' => $currUsageMonth.'GB' ]); 
                                    }
                                }
                            }else{  // not found users
                                $responseMonthQuotaAfter = [];
    ...
    public function dashboard(Request $request){
        ...
        if(  $request->system && $request->token && DB::table("$request->system.admins")->where('mobileapp_token', $request->token)->count() > 0 ){
            ...
            if(isset($getLast7DayData)){$getLast7DayData = array_values($getLast7DayData);}
            else{$getLast7DayData = [];}
            ...
            // remove kes to be able to get the values by index in next phase
            if(isset($getLast30DaysData)){$getLast30DaysData = array_values($getLast30DaysData);}
            else{$getLast30DaysData = [];}
            ...
            // remove kes to be able to get the values by index in next phase
            if(isset($getLast3Months)){$getLast3Months = array_values($getLast3Months);}
            else{$getLast3Months = [];}
    ...
    //////////////////////////////// Guest Hotels Mobile App ///////////////////////////////////
    // this function called automatically when Mobile App started and there is no token detected
    public function getUserByOnlineIpOrMac(Request $request){
        ...
        if( $request->system ){
            ...
            $session = DB::table($request->system.'.hosts')->where('address', $ip)->OrWhere('mac', $mac)->first();
            if(isset($session)){
                if( isset($session->u_id) and $session->u_id!="" and $session->u_id!=null and $session->u_id!=0){
                    
                    $user = DB::table($request->system.'.users')->where('u_id', $session->u_id)->first();
                    $whatsappClass = new App\Http\Controllers\Integrations\WhatsApp();
                    $autoReplyMsg = $whatsappClass->getAllHotelGuestInfoToAdmin($request->system, $session->u_id, $created_at, '1' );
                    
                    return json_encode(array(
                        'state' => 1,
                        'userId' => $user->u_id,
                        'userName' => $user->u_name,
                        'userUname' => $user->u_uname,
                        'allUserInfo' => $autoReplyMsg,
                        'message' =>'user founded.'));
                }else{
                    return json_encode(array('state' => 0, 'message' =>'this device is online but not assigned to any user.'));
                }
            }else{
                return json_encode(array('state' => 0, 'message' =>'IP or Mac not found.'));    
            }
